...
var eclipseShadow = function(map, projection, sliderElement, layerControlElement) {
    ...
    var getViewReset = function(path, overlay, layer, features) {
        return function() {
            var bounds = path.bounds(features),
            topLeft = bounds[0],
            bottomRight = bounds[1];

            overlay.attr('width', bottomRight[0] - topLeft[0])
            ...
            console.debug('viewreset');
        };
    };

    var renderEclipsePath = function(map, projection, eclipsePath) {
        var path = d3.geo.path().projection(projection);
        var overlay = d3.select(map.getPanes().overlayPane).append('svg');
        var layer = overlay.append('g');
        var viewreset = getViewReset(path, overlay, layer, eclipsePath);
    ...
    var renderEclipseShadow = function(map, projection, eclipseShadow) {
        ...
        var overlay = d3.select(map.getPanes().overlayPane).append('svg');
        var layer = overlay.append('g');
        var viewreset = getViewReset(path, overlay, layer, eclipseShadow);
        ...
        layer
            ...
            .attr('fill', function(d) {
                var feature = d;
                var maxAlpha = 0.9;
                var magnitude = parseFloat(feature.properties['magnitude_max']);
